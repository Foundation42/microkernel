...
static const char *TEST_SOCK = "/tmp/mk_test_transport.sock";

static int test_send_recv_simple(void) {
    transport_t *server = transport_unix_listen(TEST_SOCK, 2);
    ASSERT_NOT_NULL(server);

    transport_t *client = transport_unix_connect(TEST_SOCK, 1);
    ...
    /* Give a tiny moment for data to arrive */
    usleep(1000);
...
static int test_destroy_cleans_up_socket(void) {
    transport_t *server = transport_unix_listen(TEST_SOCK, 2);
    ASSERT_NOT_NULL(server);

    /* Socket file should exist */
    struct stat st;
    ASSERT_EQ(stat(TEST_SOCK, &st), 0);

    server->destroy(server);

    /* Socket file should be gone */
    ASSERT_NE(stat(TEST_SOCK, &st), 0);
    return 0;
}

int main(void) {
    printf("test_transport_unix:\n");
    ...
    RUN_TEST(test_destroy_cleans_up_socket);
    TEST_REPORT();
}
