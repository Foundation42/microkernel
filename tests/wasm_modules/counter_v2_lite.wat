(module
  ...
  (func (export "handle_message") (param $type i32) (param $source i64) (param $payload i32) (param $size i32) (result i32)
    ;; MSG_INCREMENT (213): count += 10  (v2!)
    (if (i32.eq (local.get $type) (i32.const 213))
      (then
        (global.set $count (i32.add (global.get $count) (i32.const 10)))
        (return (i32.const 1))
      )
    )
    ...
    ;; MSG_GET_VERSION (214): reply with version 2
    (if (i32.eq (local.get $type) (i32.const 214))
      (then
        (drop (call $mk_send_u32 (local.get $source) (i32.const 215) (i32.const 2)))
        (return (i32.const 1))
      )
    )
    ;; MSG_STOP (0): exit
